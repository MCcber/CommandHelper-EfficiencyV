...
namespace cbhk_environment.Generators.ItemGenerator.Components
{
    public class ItemPageDataContext:ObservableObject
    {
        ...
        public ObservableCollection<IconComboBoxItem> EffectItems { get; set; } = new();
        //白色画刷
        ...
        public async void ItemPages_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            {
                ItemPages parent = sender as ItemPages;
                await parent.Dispatcher.InvokeAsync(() =>
                {
                    object obj = new();
                    BindingOperations.EnableCollectionSynchronization(ItemIds, obj);
                    if (ItemIds.Count == 0)
                    ...
                    BindingOperations.EnableCollectionSynchronization(EffectItems, obj);
        ...
        private void run_command()
        {
            ...
            nbt.Append(common?.Result + function?.Result + data?.Result + (!Give && SelectedVersion == "1.13+" && data?.ItemDamage.Value > 0 ? "Damage:" + data?.ItemDamage.Value : ""));
            if (nbt.ToString().EndsWith(','))
                nbt.Remove(nbt.ToString().Length - 1, 1);
        ...
        private void ClearUnnecessaryDataCommand()
        {
            ...
            {
                Grid grid = specialDataDictionary[SelectedItemId.ComboBoxItemId];
                specialDataDictionary.Clear();
                specialDataDictionary.Add(SelectedItemId.ComboBoxItemId, grid);
            }
        }
