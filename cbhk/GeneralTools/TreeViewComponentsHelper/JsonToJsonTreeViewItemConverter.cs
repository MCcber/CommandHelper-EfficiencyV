    ...
    public class JsonToJsonTreeViewItemConverter : IValueConverter
    {
        ...
        public static JsonTreeViewDataStructure RecursivelyTraverseEachMember(JsonTreeViewDataStructure result, JArray array, int lineNumber, int layerCount, CompoundJsonTreeViewItem ParentItem = null, JsonTreeViewItem Last = null, bool isProcessingTemplate = false)
        {
            foreach (var token in array)
            {
                    ...
                    foreach (var dataType in tagsToken.Select(item => item.ToString()))
                    {
                        ...
                        switch (dataType)
                        {
                            case "TAG_Bool":
                                {
                                    IsSimpleItem = true;
                                    item.DataType = Enums.DataTypes.Bool;
                                    break;
                                }
                            case "TAG_Byte":
                            case "TAG_Short":
                            ...
                            case "TAG_String":
                                {
                                    IsSimpleItem = true;
                                    item.DataType = Enums.DataTypes.Input;
                                    if (dataType == "TAG_String")
                                        item.DataType = Enums.DataTypes.String;
                                    break;
                                }
                        }
                        if (!IsSimpleItem)
                                ...
                                case Enums.DataTypes.Long:
                                    {
                                        if (token["key"] is JToken keyToken)
                                        {
                                            ...
                                            if (token["defaultValue"] is JToken defaultValueToken)
                                            {
                                                ...
                                                if (item.DataType is not Enums.DataTypes.String)
                                ...
                                case "TAG_ItemTag":
                                    {
                                        if (token["key"] is JToken keyToken)
                                        {
                                            ...
                                            #region 处理数据类型、填充数据集
                                            SetTypeCompoundItem.DataType = Enums.DataTypes.Enum;
                                            if (token["enumList"] is JArray valueList)
                                            {
                                                foreach (var enumValue in valueList)
                                                {
                                                    SetTypeCompoundItem.EnumItemsSource.Add(new CustomControls.TextComboBoxItem() { Text = enumValue.ToString() });
                                                }
                                            }
                                            else
                                            {
                                                if (dataType == "TAG_BlockTag")
                                                {
                                                    ...
                                                    {
                                                        SetTypeCompoundItem.EnumItemsSource.Add(new CustomControls.TextComboBoxItem() { Text = blockTag });
                                                    }
                                                }
                                                else
                                                {
                                                    ...
                                                    {
                                                        SetTypeCompoundItem.EnumItemsSource.Add(new CustomControls.TextComboBoxItem() { Text = compoundTag });
                                                    }
                                                }
                                            }
                                            #endregion
                                            ...
                                            if (token["defaultValue"] is JToken defaultValueToken)
                                            {
                                                ...
                                                if (bool.TryParse(defaultValue, out bool boolValue))
                                                {
                                                    SetTypeCompoundItem.DataType = Enums.DataTypes.Bool;
                                                ...
                                                {
                                                    SetTypeCompoundItem.DataType = Enums.DataTypes.Int;
                                                    SetTypeCompoundItem.Value = intValue;
                                                    result.ResultString.Append(": " + intValue);
                                                }
                                                else
                                                {
                                                    SetTypeCompoundItem.DataType = Enums.DataTypes.String;
                                ...
                                case "TAG_CustomCompound":
                                    {
                                        ...
                                        {
                                            SetTypeCompoundItem.DataType = dataType == "TAG_NullableCompound" ? Enums.DataTypes.NullableCompound : Enums.DataTypes.OptionalCompound;
                                        }
                                        else
                                        if(dataType == "TAG_CustomCompound")
                                        {
                                            SetTypeCompoundItem.DataType = Enums.DataTypes.CustomCompound;
                                        }
                                        else
                                        {
                                            SetTypeCompoundItem.DataType = Enums.DataTypes.Compound;
                                        }
                                        #endregion
                                        ...
                                        if (SetTypeCompoundItem.DataType is not Enums.DataTypes.OptionalCompound)
                                        {
                                            result.ResultString.Append(new string(' ', layerCount * 2));
                                ...
                                case "TAG_Array":
                                    {
                                        if (token["key"] is JToken keyToken)
                                        {
                                            ...
                                            SetTypeCompoundItem.DataType = Enums.DataTypes.Array;
                                            if (dataType == "TAG_InnerArray")
                                                SetTypeCompoundItem.DataType = Enums.DataTypes.InnerArray;
                                            SetTypeCompoundItem.Key = keyToken.ToString();
                                            SetTypeCompoundItem.StartLineNumber = SetTypeCompoundItem.EndLineNumber = lineNumber;
                                            if(Last is CompoundJsonTreeViewItem lastCompletedCompoundItem && (lastCompletedCompoundItem.DataType is Enums.DataTypes.OptionalCompound || lastCompletedCompoundItem.DataType is Enums.DataTypes.NullableCompound || lastCompletedCompoundItem.DataType is Enums.DataTypes.Compound || lastCompletedCompoundItem.DataType is Enums.DataTypes.CustomCompound))
                                            {
                                ...
                                case "TAG_BlockStateProvider":
                                    {
                                        if (token["key"] is JToken keyToken && token["dataTypeList"] is JArray dataTypeList)
                                        {
                                            ...
                                            if (plan.ValueProviderContextDictionary.TryGetValue(dataTypeList[0].ToString().Replace("TAG_", ""), out CompoundJsonTreeViewItem currentCompoundItem))
                                            {
                                                ...
                                                SetTypeCompoundItem.DataType = Enums.DataTypes.ValueProvider;
                                            ...
                                            if (isProcessingTemplate)
                                            {
                                                SetTypeCompoundItem.Key = key;
                                                SetTypeCompoundItem.EnumBoxVisibility = Visibility.Visible;
                                                SetTypeCompoundItem.ValueProviderType = (Enums.ValueProviderTypes)Enum.Parse(typeof(Enums.ValueProviderTypes), dataTypeList[0].ToString().Replace("TAG_", ""));
                                                SetTypeCompoundItem.DataType = Enums.DataTypes.ValueProvider;
                                            }
                                        }
                                    }
                        ...
                        #region 计算可缺省参数的默认值
                        if (((item.DefaultValue is null && item.SelectedEnumItem is null) || (item is CompoundJsonTreeViewItem compoundJsonItem && compoundJsonItem.DataType is Enums.DataTypes.OptionalCompound)) && ((Last is not null && Last.StartLineNumber == lineNumber) || (Last is CompoundJsonTreeViewItem lastCompoundItem && lastCompoundItem.EndLineNumber == lineNumber) || (Last is null && lineNumber == 2)))
                        {
                            lineNumber--;
                            item.StartLineNumber = lineNumber;
                ...
                if (token["dataSource"] is JToken dataSourceToken && item is CompoundJsonTreeViewItem CustomCompound && CustomCompound.DataType is Enums.DataTypes.CustomCompound)
                {
                ...
                if (token["children"] is JArray children && children.Count > 0 && item is CompoundJsonTreeViewItem currentCompound)
                {
                    ...
                    if (currentCompound.DataType is not Enums.DataTypes.OptionalCompound && (currentCompound.DefaultValue is not null || currentCompound.SelectedEnumItem is not null))
                    {
                    ...
                    bool addToSwitch = currentCompound.DataType is Enums.DataTypes.Array || currentCompound.DataType is Enums.DataTypes.InnerArray || currentCompound.DataType is Enums.DataTypes.OptionalCompound;
                ...
                if (ParentItem is not null && item.Parent is null)
                {
                    ...
                    if (ParentItem.DataType is not Enums.DataTypes.Array)
