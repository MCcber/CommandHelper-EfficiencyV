    ...
    public class TreeViewRuleReader
    {
        ...
        public static ObservableCollection<JsonTreeViewItem> RecursivelyUpdateMemberLayer(CompoundJsonTreeViewItem Parent, Dictionary<string, JsonTreeViewItem> dictionary, ObservableCollection<JsonTreeViewItem> list, int lineNumber, int layer = 2)
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                //判断是否为复合数据类型
                if (list[i] is CompoundJsonTreeViewItem compoundJsonTreeViewItem && (compoundJsonTreeViewItem.DataType is cbhk.CustomControls.JsonTreeViewComponents.Enums.DataTypes.Array || compoundJsonTreeViewItem.DataType is cbhk.CustomControls.JsonTreeViewComponents.Enums.DataTypes.Compound || compoundJsonTreeViewItem.DataType is cbhk.CustomControls.JsonTreeViewComponents.Enums.DataTypes.EnumCompound || compoundJsonTreeViewItem.DataType is cbhk.CustomControls.JsonTreeViewComponents.Enums.DataTypes.MultiType || compoundJsonTreeViewItem.DataType is cbhk.CustomControls.JsonTreeViewComponents.Enums.DataTypes.ValueProvider))
                ...
                if (ParentCompoundHead.Length > 0 && list[i].Last is null)
                    currentLayerCount += 1;
                if ((list[i].Parent.DataType is cbhk.CustomControls.JsonTreeViewComponents.Enums.DataTypes.Array || list[i].Parent.DataType is cbhk.CustomControls.JsonTreeViewComponents.Enums.DataTypes.ArrayElement) && list[i].Last is null)
                    currentLayerCount += 1;
                ...
                foreach (var obj in valueProviderArray.Cast<JObject>())
                {
                    ...
                    if (obj["key"] is JToken keyToken)
                    {
                        ...
                        CompoundJsonTreeViewItem jsonTreeViewItem = new(null, null)
                        {
                            ...
                            DataType = cbhk.CustomControls.JsonTreeViewComponents.Enums.DataTypes.ValueProvider,
                            Plan = JsonToJsonTreeViewItemConverter.plan
                        };

                        if (Enum.TryParse(keyToken.ToString(), true, out cbhk.CustomControls.JsonTreeViewComponents.Enums.ValueProviderTypes valueProviderType))
